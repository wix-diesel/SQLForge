...
{
    public static readonly EnvironmentTag Production = new("production", "本番", EnvironmentSeverity.Critical);
    public static readonly EnvironmentTag Staging = new("staging", "ステージング", EnvironmentSeverity.Elevated);
    public static readonly EnvironmentTag Verification = new("verification", "検証", EnvironmentSeverity.Elevated);
    public static readonly EnvironmentTag Local = new("local", "ローカル", EnvironmentSeverity.Normal);
    public static readonly EnvironmentTag Unclassified = new("unclassified", "未分類", EnvironmentSeverity.Neutral);

    /// <summary>選択肢として提示する順序。接続ダイアログの色見本の並びと一致する。</summary>
    public static IReadOnlyList<EnvironmentTag> All { get; } =
        new[] { Production, Staging, Local, Verification, Unclassified };

    private EnvironmentTag(string id, string displayName, EnvironmentSeverity severity)
    {
        Id = id;
