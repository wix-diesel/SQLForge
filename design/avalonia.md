...
- **フェーズ 1**: 接続 / エクスプローラー / エディタ / 結果グリッド / 設定。これで「動く SQL クライアント」になる。
- **フェーズ 2**: テーブル設計と未適用の変更キュー。SSMS 相当を名乗れる分岐点。
- **フェーズ 3**: 実行プラン、次いで ER 図。

デザイン側はフェーズ 1 の範囲だけでも成立するように、3 ペインの骨格と密度を全画面で共通にしてある。
