...
**クエリも実際に実行する。**ツリーでテーブルを右クリックして「クエリを実行」を選ぶと、
右の作業領域にエディタが開いて `SELECT TOP (1000) * FROM [db].[schema].[table];` が用意される。
書き換えて「実行」（F5 / Ctrl+Enter）を押すと、下の結果ペインにグリッドで結果が出る。
データベースを右クリックすれば、空のエディタをそのデータベース向けに開ける。
...
| クエリの実行 | 動く。複数の結果セット、影響行数、実行時間、取得上限 1,000 行での打ち切り、実行の取り消し |
...
ツリーのテーブルを右クリック →「クエリを実行」で作業領域が開く。文面は用意するだけで、
**勝手には投げない**（SSMS の「上位 N 行の選択」と違うのは意図的で、何が走るか読んでから押せるようにしている）。
...
**読み取り専用で開いた接続では、書き込みの文面をサーバーへ送る前に弾く**（`AccessMode.ReadOnly`）。
判定は `ReadOnlyQueryGuard` がキーワードの有無だけで行い、文字列リテラル・引用符付き識別子・コメントの中は
先に潰してから数える。先頭の 1 文だけでなく文面全体を見るのは、`SELECT 1; DELETE FROM t` を通さないため。

これは**事故を止めるための歯止めであって、権限の代わりではない**。動的 SQL のように文面からは
分からない書き込みは通ってしまうので、本当に書かせたくない接続にはサーバー側で読み取り専用の
ロールを与えること。
...
3. `AdoDatabaseSession` を継承してカタログの読み方を 3 つ埋め
   ...
   実行先の切り替え方（`SwitchDatabaseAsync`）とテーブルをのぞく文面（`BuildTableQuery`）を書く。
