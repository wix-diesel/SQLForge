...
| 行の追加・削除 | 未実装。編集グリッドでできるのは既存の行の書き換えだけ |
...
SQLForge.Domain          接続情報のモデルと規則（環境タグ、ドライバー、接続 URL の組み立て）と、
                         ...
                         セキュリティのモデル（データベース ユーザー・サーバー ログイン・
                         ロール・権限の
                         種類・名前・定義）と、
                         クエリ結果のモデル（列 / 行 / 結果セット）と、
                         編集のモデル（編集できる列 / 先頭 N 行 / セル 1 つの書き換え）
      ↑
...
| `Views/TableEditorPane.axaml` | 先頭 100 行の編集グリッド。セルは「表示」と「入力欄」の 2 枚を重ね、開いた側だけを出す |
